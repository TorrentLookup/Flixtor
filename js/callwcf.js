...
var getEpisodeTorrents = function (episodeId, max, callback) {
    var BasicHttpBinding = require('wcf.js').BasicHttpBinding,
        ...
        "<Header />" +
        "<Body>" +
        "<JSONEpisodeTorrents xmlns='http://tempuri.org/'>" +
    ...
    proxy.send(message, "http://tempuri.org/ITorrentService/JSONEpisodeTorrents", function (response, ctx) {
        var results = $(response).find("JSONEpisodeTorrentsResult").text();
        results = results.replace(/(\r\n|\n|\r)/gm, "");
